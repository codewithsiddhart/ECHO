...
function _initModeSelect() {
    ...
    document.querySelectorAll(".mode-card").forEach(btn => {
        ...
        btn.addEventListener("click", () => {
            ...
            applyModeTheme(getMode());
            _showStoredBest();
            // set data-mode on body for CSS-driven declutter
            document.body.dataset.mode = id === "practice" ? "easy" : (id === "daily" ? "medium" : id);
            // show first-play guide on easy (hide after first tap)
            _setupFirstPlayGuide(id);
            modeScreen.classList.add("hidden");
            _startWithCountdown();
        });
    });
...
function _setupFirstPlayGuide(modeId) {
    const guide = document.getElementById("first-play-guide");
    if (!guide) return;
    if (modeId === "easy" || modeId === "practice") {
        guide.classList.remove("hidden");
        const circle = document.getElementById("circle");
        const hideGuide = () => { guide.classList.add("hidden"); };
        circle?.addEventListener("pointerdown", hideGuide, { once: true });
        circle?.addEventListener("touchstart", hideGuide, { once: true });
        setTimeout(() => guide.classList.add("hidden"), 10000);
    } else {
